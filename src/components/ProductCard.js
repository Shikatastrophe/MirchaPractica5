  ...
  return (
    ...
    <div className='w-full border-[1px] px-2 py-4'>
      ...
      <div className='flex gap-2 justify-end relative overflow-hidden w-28 text-sm'>
       ...
       })) & toast.success(`${product.title} is added`) } className='absolute z-20 w-[100px] text-gray-500 hover:text-gray-900 flex items-center gap-1 top-0 transform -translate-x-32 group-hover:translate-x-0 transition-transform cursor-pointer duration-500'>
        add to cart </p>
      </div>
      </div>
      <div><p>{product.category}</p></div>
      <div className='absolute top-4 right-0'>
        {product.isNew &&<p className='bg-black text-white font-semibold font-titleFont px-6 py-1'>Sale</p>}</div>
    </div>
    <ToastContainer
        position="top-left"
        autoClose={2000}
